...
executor = DockerCommandLineCodeExecutor(
    ...
    work_dir='DockerSpace'  # Use the temporary directory to store the code files.
)

# Create an agent with code executor configuration that uses docker.
code_executor_agent_using_docker = ConversableAgent(
    name="code_executor_agent_docker",
    llm_config=azure_OAI,  # Turn off LLM for this agent.
    code_execution_config={"executor": executor},  # Use the docker command line code executor.
    human_input_mode="TERNATE",  # Always take human input for this agent for safety.
    is_termination_msg=lambda msg: "Bye!" or "Good Bye!" or "TERMINATE" in msg["content"],  # Terminate the conversation if the user sends 'TERMINATE
)
